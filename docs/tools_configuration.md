...
Octa's tools configuration is located in the `tools` field of `config.json`.
...
By default, Octa blocks the following dangerous commands:
...
All configuration options can be overridden via environment variables with the format `OCTA_TOOLS_<SECTION>_<KEY>`:

For example:
- `OCTA_TOOLS_WEB_BRAVE_ENABLED=true`
- `OCTA_TOOLS_EXEC_ENABLE_DENY_PATTERNS=false`
- `OCTA_TOOLS_CRON_EXEC_TIMEOUT_MINUTES=10`
