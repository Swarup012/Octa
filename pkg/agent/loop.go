...
func (al *AgentLoop) runLLMIteration(
	...
	for iteration < agent.MaxIterations {
		...
		// Execute tool calls — parallel when multiple tools requested in one iteration.
		type toolExecResult struct {
			tc     providers.ToolCall
			result *tools.ToolResult
		}
		execResults := make([]toolExecResult, len(normalizedToolCalls))

		if len(normalizedToolCalls) == 1 {
			// Fast path: single tool
			tc := normalizedToolCalls[0]
			argsJSON, _ := json.Marshal(tc.Arguments)
			argsPreview := utils.Truncate(string(argsJSON), 200)
			logger.InfoCF("agent", fmt.Sprintf("Tool call: %s(%s)", tc.Name, argsPreview),
				map[string]any{"agent_id": agent.ID, "tool": tc.Name, "iteration": iteration})

			asyncCallback := func(callbackCtx context.Context, result *tools.ToolResult) {
				if !result.Silent && result.ForUser != "" {
					logger.InfoCF("agent", "Async tool completed, agent will handle notification",
						map[string]any{"tool": tc.Name, "content_len": len(result.ForUser)})
				}
			}

			execResults[0] = toolExecResult{
				tc:     tc,
				result: agent.Tools.ExecuteWithContext(ctx, tc.Name, tc.Arguments, opts.Channel, opts.ChatID, asyncCallback),
			}
		} else {
			logger.InfoCF("agent", "Executing tool calls in parallel",
				map[string]any{"agent_id": agent.ID, "count": len(normalizedToolCalls), "iteration": iteration})
		...
		for _, er := range execResults {
			...
			if len(toolResult.Media) > 0 && opts.SendResponse {
				...
				al.bus.PublishOutboundMedia(ctx, bus.OutboundMediaMessage{Channel: opts.Channel, ChatID: opts.ChatID, Parts: parts})
			}
