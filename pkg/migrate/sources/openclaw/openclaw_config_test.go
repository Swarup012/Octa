...
func TestLoadOpenClawConfig(t *testing.T) {
	...
	if workspace != "~/.octa/workspace" {
		t.Errorf("expected workspace '~/.octa/workspace', got '%s'", workspace)
	}
...
func TestConvertToOcta(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, warnings, err := cfg.ConvertToOcta("")
	if err != nil {
	...
	if picoCfg.Agents.Defaults.Workspace != "~/.octa/workspace" {
		t.Errorf("expected workspace '~/.octa/workspace', got '%s'", picoCfg.Agents.Defaults.Workspace)
	}
...
func TestConvertToOctaWithQQAndDingTalk(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, _, err := cfg.ConvertToOcta("")
	if err != nil {
		t.Fatalf("failed to convert config: %v", err)
	}
...
func TestToStandardConfig(t *testing.T) {
	...
	if stdCfg.Agents.Defaults.Workspace != "~/.octa/workspace" {
		t.Errorf("expected workspace '~/.octa/workspace', got '%s'", stdCfg.Agents.Defaults.Workspace)
	}
