...
func (t *CronTool) Description() string {
	return "Schedule one-time or recurring reminders and tasks. Actions: add, list, remove, enable, disable. For add: provide action=add, message=reminder text, and one of: at_seconds (seconds from now, e.g. 120 for 2 minutes), every_seconds (recurring interval), or cron_expr (cron expression). Example: {action: add, message: hello, at_seconds: 120}"
}
