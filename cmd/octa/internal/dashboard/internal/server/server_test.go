...
func TestGetConfig_MissingFile_ReturnsDefault(t *testing.T) {
	mux := http.NewServeMux()
	RegisterConfigAPI(mux, "/tmp/nonexistent-octa-launcher-test/config.json")
...
func TestDefaultConfigPath(t *testing.T) {
	...
	if !strings.HasSuffix(path, filepath.Join(".octa", "config.json")) {
		t.Errorf("expected path ending with .octa/config.json, got %q", path)
	}
}
