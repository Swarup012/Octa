...
// RegisterProcessAPI registers endpoints to start, stop and check status of the octa gateway.
...
func handleStartGateway(w http.ResponseWriter, r *http.Request) {
	...
	if exe, err := os.Executable(); err == nil {
		...
		candidate := filepath.Join(dir, "octa")
	...
	if err := cmd.Start(); err != nil {
		log.Printf("Failed to start octa gateway: %v\n", err)
		http.Error(w, fmt.Sprintf("Failed to start gateway: %v", err), http.StatusInternalServerError)
		return
	}
	...
	log.Printf("Started octa gateway (PID: %d) from %s\n", cmd.Process.Pid, execPath)
...
func handleStopGateway(w http.ResponseWriter, r *http.Request) {
	...
	if runtime.GOOS == "windows" {
		...
		// Linux/macOS
		err = exec.Command("pkill", "-f", "octa gateway").Run()
	}
	...
	log.Printf("Stopped octa gateway processes.\n")
	w.Header().Set("Content-Type", "application/json")
