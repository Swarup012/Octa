...
A standalone launcher for Octa, providing visual JSON editing and OAuth provider authentication management.
...
# Run with default config path (~/.octa/config.json)
./octa-launcher
...
```
Usage: octa-config [options] [config.json]

Arguments:
  config.json    Path to the configuration file (default: ~/.octa/config.json)
...
After authentication, Google redirects to `GET /auth/callback`, which saves the credentials and redirects back to the octa-config UI.
...
Gets the running status of the `octa gateway` process.
...
Starts the `octa gateway` process in the background.
...
Stops the running `octa gateway` process.
